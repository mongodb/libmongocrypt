...
class MongoCrypt(object):
    ...
    def explicit_encryption_context(self, document, key_id, algorithm):
        """Creates a context to use for explicit encryption.

        :Parameters:
          - `document`: The encoded document to encrypt, which must be in the
        ...
        ctx = self._create_context()
        wrapped = MongoCryptContext(ctx)
        with MongoCryptBinaryIn(key_id) as binary:
            if not lib.mongocrypt_ctx_setopt_key_id(ctx, binary.bin):
                wrapped._raise_from_status()

        with MongoCryptBinaryIn(algorithm) as binary:
            if not lib.mongocrypt_ctx_setopt_key_algorithm(
                    ctx, binary.bin):
                wrapped._raise_from_status()
    ...
    def explicit_decryption_context(self, document):
        ...
        wrapped = MongoCryptContext(ctx)
        with MongoCryptBinaryIn(document) as binary:
            if not lib.mongocrypt_ctx_explicit_decrypt_init(
                    ctx, binary.bin):
                wrapped._raise_from_status()
        return wrapped
        ...
        if kms_provider == 'aws':
            if opts is None or opts.master_key is None:
                raise ValueError('opts.master_key is required for '
                                 'kms_provider: "aws"')
            if 'region' not in opts.master_key or 'key' not in opts.master_key:
                raise ValueError('opts.master_key must include "region" and '
