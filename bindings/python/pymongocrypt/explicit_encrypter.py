...
from pymongocrypt.state_machine import run_state_machine, a_run_state_machine
...
class ExplicitEncrypter(object):
    ...
    async def a_create_data_key(self, kms_provider, master_key=None,
                        key_alt_names=None, key_material=None):
        """Creates a data key used for explicit encryption.
        ...
        opts = DataKeyOpts(master_key, encoded_names, key_material)
        with self.mongocrypt.data_key_context(kms_provider, opts) as ctx:
            key = await a_run_state_machine(ctx, self.callback)
    ...
    async def a_rewrap_many_data_key(self, filter, provider=None, master_key=None):
        """Decrypts and encrypts all matching data keys with a possibly new `master_key` value.
    ...
    async def a_encrypt(self, value, algorithm, key_id=None, key_alt_name=None,
                query_type=None, contention_factor=None, range_opts=None,
                is_expression=False):
        """Encrypts a BSON value.
    ...
    async def a_decrypt(self, value):
        """Decrypts a BSON value.
