...
static void
_mc_token_test_cleanup (_mc_token_test *test)
{
...
static void
_mc_token_test_run (_mongocrypt_tester_t *tester, const char *path)
{
...
#define CHECK_FIELD(f) \
   ASSERT_OR_PRINT_MSG (test.f.data, "Missing field '" #f "' in test");
...
#define TEST_COLL_TOKEN(Name)                                               \
   mc_##Name##Token_t *Name##Token =                                        \
      mc_##Name##Token_new (crypt->crypto, collectionsLevel1Token, status); \
...
#define TEST_DERIVED(Name)                                                    \
   mc_##Name##DerivedFromDataToken_t *Name##DerivedFromDataToken =            \
      mc_##Name##DerivedFromDataToken_new (                                   \
         crypt->crypto, Name##Token, &test.value, status);                    \
   ASSERT_OR_PRINT (Name##DerivedFromDataToken, status);                      \
   ASSERT_CMPBUF (                                                            \
      *mc_##Name##DerivedFromDataToken_get (Name##DerivedFromDataToken),      \
      test.Name##DerivedFromDataToken);                                       \
   mc_##Name##DerivedFromDataTokenAndCounter_t                                \
      *Name##DerivedFromDataTokenAndCounter =                                 \
         mc_##Name##DerivedFromDataTokenAndCounter_new (                      \
            crypt->crypto, Name##DerivedFromDataToken, test.counter, status); \
   ASSERT_OR_PRINT (Name##DerivedFromDataTokenAndCounter, status);            \
   ASSERT_CMPBUF (*mc_##Name##DerivedFromDataTokenAndCounter_get (            \
                     Name##DerivedFromDataTokenAndCounter),                   \
