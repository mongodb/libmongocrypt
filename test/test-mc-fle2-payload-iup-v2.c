...
static void
_test_FLE2InsertUpdatePayloadV2_parse (_mongocrypt_tester_t *tester)
{
   ...
   _mongocrypt_buffer_copy_from_hex (&expect_indexKeyId,
                                     "4c2352e235314b429c897d833b411b77");
   ...
   _mongocrypt_buffer_copy_from_hex (
    ...
    _mongocrypt_buffer_copy_from_hex (
      &expect_serverDerivedFromDataToken,
   ...
   _mongocrypt_buffer_copy_from_hex (&expect_userKeyId,
...

    {
        ...
        ASSERT_OK_STATUS (mc_FLE2InsertUpdatePayloadV2_parse (&iup, &input, status),
                          status);
...
static void
_test_mc_FLE2InsertUpdatePayloadV2_decrypt (_mongocrypt_tester_t *tester)
{
   _mongocrypt_buffer_t input;
   mc_FLE2InsertUpdatePayloadV2_t iup;
   ...
   ASSERT (_mongocrypt_buffer_copy_from_data_and_size (
      &expect_plaintext, (const uint8_t *) "\x08\x00\x00\x00shreyas", 12));
   ...
   /* Test successful decrypt. */
   {
      ...
      ASSERT_OK_STATUS (mc_FLE2InsertUpdatePayloadV2_parse (&iup, &input, status),
                        status);
      const _mongocrypt_buffer_t *got = mc_FLE2InsertUpdatePayloadV2_decrypt (
