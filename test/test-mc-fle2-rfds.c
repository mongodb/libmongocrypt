...
static void
test_mc_FLE2RangeFindDriverSpec_parse (_mongocrypt_tester_t *tester)
{
   typedef struct {
      const char *desc;
      const char *in;
   ...
   testcase tests[] = {
      ...
      {.desc = "Missing top $and",
       .in = RAW_STRING ({"foo" : "bar"}),
       .expectError = "error unable to find '$and'"},
      {.desc = "Empty document",
       .in = RAW_STRING ({}),
       .expectError = "error unable to find '$and'"},
   };
   ...
   for (size_t i = 0; i < sizeof (tests) / sizeof (tests[0]); i++) {
      ...
      bool ret =
         mc_FLE2RangeFindDriverSpec_parse (&rfds, TMP_BSON (test->in), status);
      if (!test->expectError) {
         ...
         if (test->expect.lower.set) {
            ASSERT_CMPINT32 (test->expect.lower.value,
                             ==,
                             bson_iter_int32 (&rfds.lower.value));
            ASSERT_CMPINT (
               test->expect.lower.included, ==, rfds.lower.included);
         }
         ASSERT_CMPINT (test->expect.upper.set, ==, rfds.upper.set);
         if (test->expect.upper.set) {
            ASSERT_CMPINT32 (test->expect.upper.value,
                             ==,
                             bson_iter_int32 (&rfds.upper.value));
            ASSERT_CMPINT (
               test->expect.upper.included, ==, rfds.upper.included);
         }
         ASSERT_CMPINT (
            test->expect.isAggregateExpression, ==, rfds.isAggregateExpression);
      } else {
...
static bson_iter_t
tmp_iter (_mongocrypt_tester_t *tester, int32_t v)
{
...
static void
test_mc_FLE2RangeFindDriverSpec_to_placeholders (_mongocrypt_tester_t *tester)
{
   ...
   _mongocrypt_buffer_copy_from_hex (&user_key_id,
                                     "0123456789abcdefedcba98765432101");
   _mongocrypt_buffer_copy_from_hex (&index_key_id,
                                     "abcdefabcdabcdefedcba98765432101");
   ...
   testcase_t tests[] = {
      {.desc = "Match Expression with both inclusive bounds",
       ...
       .expected = RAW_STRING ({
          "$and" : [
             {"age" : {"$gte" : "<placeholder1>"}},
             {"age" : {"$lte" : "<placeholder2>"}}
          ]
       })},
      {.desc = "Match Expression with one inclusive bound",
       ...
       .p1 = {.lowerBound = TMP_ITER (23),
              ...
              .firstOp = FLE2RangeOperator_kGte},
       .expected =
          RAW_STRING ({"$and" : [ {"age" : {"$gte" : "<placeholder1>"}} ]})},
      {.desc = "Match Expression with both exclusive bounds",
       .in = RAW_STRING (
          {"$and" : [ {"age" : {"$gt" : 23}}, {"age" : {"$lt" : 35}} ]}),
       .p1 = {.lowerBound = TMP_ITER (23),
              .lbIncluded = false,
              .upperBound = TMP_ITER (35),
              .ubIncluded = false,
      ...
      {.desc = "Match Expression with one exclusive bound",
       ...
       .expected =
          RAW_STRING ({"$and" : [ {"age" : {"$lt" : "<placeholder1>"}} ]})},
      {.desc = "Match Expression with flipped bounds",
       .in = RAW_STRING (
          {"$and" : [ {"age" : {"$lte" : 35}}, {"age" : {"$gte" : 23}} ]}),
       .p1 = {.lowerBound = TMP_ITER (23),
              .lbIncluded = true,
              .upperBound = TMP_ITER (35),
              .ubIncluded = true,
       ...
       .expected = RAW_STRING ({
          "$and" : [
             {"age" : {"$lte" : "<placeholder1>"}},
             {"age" : {"$gte" : "<placeholder2>"}}
          ]
       })},
      {.desc = "Aggregate Expression with both inclusive bounds",
       .in = RAW_STRING (
          {"$and" : [ {"$gte" : [ "$age", 23 ]}, {"$lte" : [ "$age", 35 ]} ]}),
       .p1 = {.lowerBound = TMP_ITER (23),
              .lbIncluded = true,
              .upperBound = TMP_ITER (35),
              .ubIncluded = true,
       ...
       .expected = RAW_STRING ({
          "$and" : [
             {"$gte" : [ "$age", "<placeholder1>" ]},
             {"$lte" : [ "$age", "<placeholder2>" ]}
          ]
       })},
      {.desc = "Aggregate Expression with one inclusive bound",
       ...
       .p1 = {.lowerBound = TMP_ITER (23),
              ...
              .firstOp = FLE2RangeOperator_kGte},
       .expected =
          RAW_STRING ({"$and" : [ {"$gte" : [ "$age", "<placeholder1>" ]} ]})},
      {.desc = "Aggregate Expression with both exclusive bounds",
       .in = RAW_STRING (
          {"$and" : [ {"$gt" : [ "$age", 23 ]}, {"$lt" : [ "$age", 35 ]} ]}),
       .p1 = {.lowerBound = TMP_ITER (23),
              .lbIncluded = false,
              .upperBound = TMP_ITER (35),
              .ubIncluded = false,
      ...
      {.desc = "Aggregate Expression with one exclusive bound",
       ...
       .p1 = {.lowerBound = negInf,
              ...
              .firstOp = FLE2RangeOperator_kLt},
       .expected =
          RAW_STRING ({"$and" : [ {"$lt" : [ "$age", "<placeholder1>" ]} ]})},
      {.desc = "Aggregate Expression with flipped bounds",
       .in = RAW_STRING (
          {"$and" : [ {"$lte" : [ "$age", 35 ]}, {"$gte" : [ "$age", 23 ]} ]}),
       .p1 = {.lowerBound = TMP_ITER (23),
              .lbIncluded = true,
              .upperBound = TMP_ITER (35),
              .ubIncluded = true,
       ...
       .expected = RAW_STRING ({
          "$and" : [
             {"$lte" : [ "$age", "<placeholder1>" ]},
             {"$gte" : [ "$age", "<placeholder2>" ]}
          ]
       })}
   ...
   bson_t *range_opts_bson =
      TMP_BSON ("{'min': %d, 'max': %d, 'sparsity': {'$numberLong': '%d'}}",
                indexMin,
                indexMax,
                sparsity);

   ASSERT_OK_STATUS (mc_RangeOpts_parse (&range_opts, range_opts_bson, status),
                     status);
   ...
   for (size_t i = 0; i < sizeof (tests) / sizeof (tests[0]); i++) {
      ...
      ASSERT_OK_STATUS (
         mc_FLE2RangeFindDriverSpec_parse (&spec, TMP_BSON (test->in), status),
         status);
      ...
      bson_t *expected;
      {
         ...
         if (test->p1.firstOp) {
            ...
            mc_makeRangeFindPlaceholder_args_t p1_args_full = {
               .isStub = test->p1.isStub,
         ...
         if (test->p2.firstOp) {
            mc_makeRangeFindPlaceholder_args_t p2_args_full = {
               .isStub = test->p2.isStub,
               .user_key_id = &user_key_id,
               .index_key_id = &index_key_id,
      ...
      bson_t out;
      bool ok =
         mc_FLE2RangeFindDriverSpec_to_placeholders (&spec,
                                                     &range_opts,
...
static void
test_mc_getNextPayloadId (_mongocrypt_tester_t *tester)
{
