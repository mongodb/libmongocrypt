...
_test_mongocrypt_buffer_from_iter (_mongocrypt_tester_t *tester)
{
   ...
   ROUNDTRIP ("str_key",
              ...
              "06 00 00 00 3F 3F 3F 3F 3F 00");
   BSON_ASSERT (
      _mongocrypt_buffer_to_bson_value (&plaintext, 0x02 /* string */, &out));
   ...
   BSON_ASSERT (
      _mongocrypt_buffer_to_bson_value (&plaintext, 0x10 /* int */, &out));
   ...
   ROUNDTRIP ("int_key", "0B 00 00 00 10 00 63 C5 54 00 00", "63 C5 54 00");
   BSON_ASSERT (
      !_mongocrypt_buffer_to_bson_value (&plaintext, 0x99 /* invalid */, &out));
