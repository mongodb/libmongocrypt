...
_test_mc_reader_uuid (_mongocrypt_tester_t *tester)
{
   ...
   ASSERT_CMPBYTES (expected_bytes, expected_len, value.data, value.len);
...
_test_mc_reader_bytes (_mongocrypt_tester_t *tester)
{
   ...
   ASSERT_OK_STATUS (
      ...
      status);
   ASSERT_CMPBYTES (expected_bytes, 4, ptr, len);

   _mongocrypt_buffer_t value_buf;
   ASSERT_OK_STATUS (mc_reader_read_buffer_to_end (&reader, &value_buf, status),
                     status);
   ASSERT_CMPBYTES (
      expected_bytes + 4, expected_len - 4, value_buf.data, value_buf.len);
