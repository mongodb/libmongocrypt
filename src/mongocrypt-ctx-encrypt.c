...
static bool explicit_encrypt_init(mongocrypt_ctx_t *ctx, mongocrypt_binary_t *msg) {
    ...
    if (ctx->opts.rangeopts.set && ctx->opts.index_type.set) {
        ...
        if (ctx->opts.index_type.value == MONGOCRYPT_INDEX_TYPE_TEXTPREVIEW) {
            return _mongocrypt_ctx_fail_w_msg(ctx, "cannot set range opts with 'textPreview' algorithm");
        }
    ...
    if (ctx->opts.textopts.set && ctx->opts.index_type.set) {
        if (ctx->opts.index_type.value != MONGOCRYPT_INDEX_TYPE_TEXTPREVIEW
            ...
            return _mongocrypt_ctx_fail_w_msg(ctx, "cannot set string opts without string or textPreview index type");
        }
    }
