...
template <typename F> deferred_fn<F> make_deferred(F fn) {
    return deferred_fn<F>{fn};
}

#define DEFER(...) make_deferred([&] __VA_ARGS__)

void bson_child(bson_t *out, bson_iter_t *it) {
    uint32_t len;
    const uint8_t *data;
...
void print_field_info(bson_t *bson, std::string path) {
    bson_iter_t iter;
...
int do_main(int argc, const char *const *argv) {
    if (argc < 2 || argc > 3) {
        print_usage();
        return 2;
    }
...
#define LOAD_SYM(Name)                                                                                                 \
    ...
    bson_error_t error;
    bson_t *input = bson_new_from_json(reinterpret_cast<const uint8_t *>(input_json.c_str()),
    ...
    if (!lib) {
        fprintf(stderr, "Failed to lib_create for csfle: %s\n", mongo_crypt_v1_status_get_explanation(status));
        return 6;
    }
    ...
    uint8_t *data_ptr = mongo_crypt_v1_analyze_query(qa,
                                                     bson_get_data(input),
                                                     doc_ns,
                                                     ...
                                                     status);
    if (!data_ptr) {
        fprintf(stderr, "Failed to analyze the given query: %s\n", mongo_crypt_v1_status_get_explanation(status));
        return 8;
    }
