...
static mongoc_stream_t *_get_aws_stream(mongoc_crypt_error_t *error) {
  int errcode;
  int r;
  ...
  if (!(r == 0 || ERRNO_IS_AGAIN(errcode))) {
    ...
    SET_CRYPT_ERR("mongoc_socket_connect unexpected return: %d (errno: %d)\n",
                  r, errcode);
    return NULL;
  }
...
static void print_without_carriage_return(uint8_t *buf, ssize_t n) {
  ssize_t i;
...
static bool _api_call(mongoc_crypt_t *crypt, kms_request_t *request,
                      kms_response_t **response, mongoc_crypt_error_t *error) {
  bool ret = false;
  ...
  if (n != (ssize_t)sreq_len) {
    SET_CRYPT_ERR("Only wrote %zd of %zu bytes (errno: %d)\n", n, sreq_len,
                  errno);
    goto cleanup;
  }
  ...
  while (kms_response_parser_wants_bytes(parser, sizeof(read_buf))) {
    ...
    n = mongoc_stream_read(stream, read_buf, sizeof(read_buf), 1, timeout_msec);
    if (n < 0) {
...
static bool _get_data_key_from_response(kms_response_t *response,
  ...
  bson_json_data_reader_ingest(reader, (const uint8_t *)raw_response_body,
                               strlen(raw_response_body));
