...
bool
mongocrypt_key_decryptor_status (mongocrypt_key_decryptor_t *kd, mongocrypt_status_t* out)
{
   BSON_ASSERT (kd);

   if (!mongocrypt_status_ok (kd->status)) {
      mongocrypt_status_copy_to (kd->status, out);
      return false;
   }
