...
/* This key cache is a very rudimentary fill-in until we design something better.
 * - keys are never removed.
 * - limited to 64 keys total.
 */

static int _cmp_uuid (_mongocrypt_buffer_t* uuid1, _mongocrypt_buffer_t* uuid2) {
...
bool _mongocrypt_keycache_add (mongocrypt_t* crypt, _mongocrypt_buffer_t* docs, uint32_t num_docs, mongocrypt_error_t** error) {
    bool ret = false;
    int i, j;
    ...
    for (i = 0; i < num_docs; i++) {
        ...
        for (j = 0; j < sizeof(crypt->keycache) / sizeof(crypt->keycache[0]); j++) {
            if (crypt->keycache[j].used && 0 == _cmp_uuid(&crypt->keycache[j].key.id, &parsed_key.id)) {
        ...
        /* Keep a copy of the key document, since we'll have non-owning references to the UUID and keymaterial. TODO: consider just copying the bits we need instead. */
...
const _mongocrypt_key_t* _mongocrypt_keycache_get_by_id (mongocrypt_t* crypt, const _mongocrypt_buffer_t* uuid, mongocrypt_error_t** error) {
    int i;

    for (i = 0; i < sizeof(crypt->keycache) / sizeof(crypt->keycache[0]); i++) {
...
void _mongocrypt_keycache_dump (mongocrypt_t* crypt) {
    int i;
    int total_used = 0;
